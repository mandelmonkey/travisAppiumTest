...
if (OS_ANDROID) {
  let testFairyAppID = Ti.App.Properties.getString("testFairyAppID", undefined)
  if (testFairyAppID != undefined && Alloy.CFG.isDevelopment) {
    let testFairy = require("com.mandelduck.testfairysdk.TestFairyLoader")
    let Activity = require('android.app.Activity');
    let activity = new Activity(Ti.Android.currentActivity);
    testFairy.startTestFairy(activity, testFairyAppID);
  }
}


globals.blockHeight = {
  testnet: Ti.App.Properties.getInt("testnet_blockheight", 1572832),
  mainnet: Ti.App.Properties.getInt("mainnet_blockheight", 588907),
}
